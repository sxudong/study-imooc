 ...
 * 使用 AsynchronousServerSocketChannel 实现
 */
public class EchoServer {
    ...
    public void start() {
        try {
            // 绑定监听端口
            // 里面有一个 AsynchronousChannelGroup ，类似一个线程池，提供一些异步的通道，可以共享的一些系统资源
            ...
            while (true) {
                ...
                // 没有其它的程序就结束了，这里使用System.in.read()阻塞在这里，
                // 使主线程不会过早的结束。避免while循环过于频繁
                System.in.read();
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    /**
     * 创建 AcceptHandler，用来处理 accept 函数的异步调用的返回结果，
     * 即接收客户端的连接请求后，进行回调
     * <p>
    ...
    private class AcceptHandler implements CompletionHandler<AsynchronousSocketChannel, Object> {
        ...
        public void completed(AsynchronousSocketChannel result, Object attachment) {
            // 服务端接着等待下一个客户端来连接的请求
            // 底层限制了 accept 里边调用 accept 的层级，保证了不会出现 stackOverflow 的错误
            if (serverChannel.isOpen()) {
                serverChannel.accept(null, this);
            }

            // 处理读写操作（连接之后，接来就是读取数据并发送）
            ...
            if (null != clientChannel && clientChannel.isOpen()) {
                ...
                // 读取客户端发送的消息到 buffer 中，并交给 ClientHandler 处理把消息转发回客户端
    ...
    /***
     * 创建客户端的 ClientHandler，用来处理 read 函数的异步调用的返回结果
     */
