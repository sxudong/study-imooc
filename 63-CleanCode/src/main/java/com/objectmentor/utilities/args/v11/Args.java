package com.objectmentor.utilities.args.v11;
...
/**
 * 14.3 p209-p210
 * 离彻底删除那 3 个旧映射的时机越来越近了。首先，修改 getBoolean 函数。
 * 接下来用同样的方法处理 String 类型和 Integer 类型参数，对 boolean
 * 参数做一点清理工作。
 */
public class Args {
    ...
    // 由于那些 parse 方法没有太多事可做，对它们进行了内联修改：
    ...
    // 为什么突然决定对 ClassCastException ？原因是我有一组单元测试，
    // 还有用 FitNesse 编写的一组验收测试。FitNesse 测试确认，如果
    // 非布尔值参数调用 getBoolean，应该返回 false。可单元测试的结果
    // 不是这样。而到此时为止，我一直只调用单元测试。
